...
impl<'a> Parser<'a> {
    ...
    fn parse_statement(&mut self) -> error::Result<Statement> {
        match self.peek_tok() {
            ...
            TokenType::BeginEnv => self.parse_environment::<false>(),
    ...
    fn parse_environment<const IS_REAL: bool>(&mut self) -> error::Result<Statement> {
        ...
        let args = self.parse_function_args::<true>(
            TokenType::Lparen,
            TokenType::Rparen,
            TokenType::Lsqbrace,
            TokenType::Rsqbrace,
    ...
    fn parse_latex_function(&mut self) -> error::Result<Statement> {
        ...
        let args = self.parse_function_args::<false>(
            TokenType::Lbrace,
            TokenType::Rbrace,
    ...
    fn parse_function_args<const IS_ENV: bool>(
        &mut self,
        open: TokenType,
        closed: TokenType,
            ...
            || self.peek_tok() == TokenType::Star
        {
            loop {
                ...
                if IS_ENV {
                    if let TokenType::Eof | TokenType::Lbrace = self.peek_tok() {
                        break;
                    }
                } else {
