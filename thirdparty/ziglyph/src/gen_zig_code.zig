...
fn writeFile(collections: []Collection, filepath: []const u8) !void {
    ...
    try writer.writeAll("// Autogenerated from https://www.unicode.org/Public/" ++ UNICODE_VERSION ++ "/ucd/\n");
...
fn processUcd() !void {
    ...
    var nfc_comp = try compressor(.raw, nfc_file.writer(), .{ .level = .best });
    ...
    var nfd_comp = try compressor(.raw, nfd_file.writer(), .{ .level = .best });
    ...
    var nfkd_comp = try compressor(.raw, nfkd_file.writer(), .{ .level = .best });
    ...
    // Headers.
    const map_header =
        \\// Autogenerated from https://www.unicode.org/Public/{s}/ucd/
        \\
        \\pub fn to{s}(cp: u21) u21 {{
        \\    return switch (cp) {{
        \\
    ;
    _ = try l_writer.print(map_header, .{ UNICODE_VERSION, "Lower" });
    _ = try t_writer.print(map_header, .{ UNICODE_VERSION, "Title" });
    _ = try u_writer.print(map_header, .{ UNICODE_VERSION, "Upper" });
    ...
    while (try reader.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        while (fields_iter.next()) |orig_field| : (field_index += 1) {
            if (field_index == 0) {
                ...
                if (select) {
                    // Mapping.
                    const trimmed_field = std.mem.trim(u8, orig_field, " ");
                    var cp_iter = std.mem.splitScalar(u8, trimmed_field, ' ');
...
fn processCombiningClass() !void {
    ...
    const header =
        ...
        \\
    ;
    _ = try writer.print(header, .{UNICODE_VERSION});
...
fn akds() !void {
    ...
    defer out_file.close();
    var comp = try compressor(.raw, out_file.writer(), .{ .level = .best });
    ...
    lines: while (try in_reader.readUntilDelimiterOrEof(&line_buf, '\n')) |raw| {
        ...
        var halves = std.mem.splitAny(u8, line, ";");
        const cps_str = std.mem.trim(u8, halves.next().?, " ");

        // Code points
        var cps_str_iter = std.mem.splitAny(u8, cps_str, " ");
        ...
        var element_str_iter = std.mem.splitAny(u8, elements_str, "][");
        i = 0;
        while (element_str_iter.next()) |element_str| : (i += 1) {
            ...
            var weight_str_iter = std.mem.splitAny(u8, trimmed_estr, ".");
            var j: usize = 0;
